...
*/
std::map<std::string, double> SingleGrades(const std::string& nombre) {
  ...
  std::string alu;
  double nota;
  std::map<std::string, double> listado;

  while (fichero_entrada >> alu >> nota) {
    if (ComprobarDatos(alu, nota)) {
      auto it = listado.find(alu);
      if (it != listado.end()) {
        if (it->second < nota) {
          it->second = nota;
        }
...
*/
void Insert(std::map<std::string, double>& listado, const std::string& alu, const double nota) {
  if (ComprobarDatos(alu, nota)) {
    auto it = listado.find(alu);
    if (it != listado.end()) {
      if (it->second < nota) {
 ...
 * @param listado map que contiene los datos almacenados
*/
void PrintMap(const std::map<std::string, double>& listado) {
  for (const auto& [alumno, nota] : listado) {
